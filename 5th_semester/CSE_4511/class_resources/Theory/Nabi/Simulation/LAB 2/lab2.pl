		      ...
		      if($input[2] eq $src_ip[$pck_id]){
				...
				$start[$pck_id] = $input[1]; 
	                        $status[$pck_id] = 0;
                      		...
                      		$total_sent++;
				print "Pckt:",$pck_id,"  Transmitted at:",$start[$pck_id],"  src-ip:port-> ", $src_ip[$pck_id],":",$src_port[$pck_id],"  dst-ip:port->", $dst_ip[$pck_id],":", $dst_port[$pck_id],"\n";
		     }
        }
        ...
        if($input[0] eq 'r' && $input[4] eq 'tcp') {
		      ...
		      if($input[3] eq $dst_ip[$pck_id]){
				...
				$finish[$pck_id] = $input[1]; 
	                        $status[$pck_id] = 1;
				$delay[$pck_id]=$finish[$pck_id]-$start[$pck_id];    			

                      		$total_recv++;
				print "Pckt:",$pck_id,"  Received at:",$finish[$pck_id],"  src-ip:port-> ", $src_ip[$pck_id],":",$src_port[$pck_id],"  dst-ip:port->", $dst_ip[$pck_id],":", $dst_port[$pck_id],"\n";
		     }
        }
...
for ($i = 0; $i <=$max_node; $i++) {     
           if ($sent[$i] > 0 ) {
               print " Source Node:",$i," -> Throughput(recved/sent): ",$recv[$i],"/", $sent[$i], "=", $recv[$i]/$sent[$i],"\n";
		#if($recv[$i]>0){print "\tAvg.delay :",$flow_delay[$s]/$recv[$s],"\n";}
           }
} 
print "\n\nAt a glance :\n";
